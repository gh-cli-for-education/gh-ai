...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -r|--regexp) REGEX="$2"; shift ;;
        -u|--url) SHOW_URL=true ;;
        -l|--login) SHOW_LOGIN=true ;;
        -w|--orgurl) SHOW_ORG_URL=true ;;
        -s|--site) SHOW_SITE=true ;;
        -o|--org) DEFAULT_ORG="$2"; shift ;;
        --default) DEFAULT_ORG="$2"; shift ;;
        -h|--help) 
            ...
            echo "  -w, --orgurl     Show GitHub user URL as a member of the organization"
            echo "  -s, --site       Show URL of the members GitHub pages websites"
...
# Use the selected or default organization
if [ -n "$DEFAULT_ORG" ]; then
    ORG="$DEFAULT_ORG"
fi

# Call Github API to get organization members info
GH_MEMBERS=$(gh api "/orgs/$ORG/members" | jq '.[]')

# Display the information based on options selected
if [ -n "$SHOW_FULLNAME" ]; then
    echo "$GH_MEMBERS" | jq -r '.full_name'
fi
...
if [ -n "$REGEX" ]; then
    echo "$GH_MEMBERS" | jq --arg regex "$REGEX" 'select(.field | test($regex))'
fi

if [ -n "$SHOW_URL" ]; then
    echo "$GH_MEMBERS" | jq -r '.url'
fi

if [ -n "$SHOW_LOGIN" ]; then
    echo "$GH_MEMBERS" | jq -r '.login'
fi

if [ -n "$SHOW_ORG_URL" ]; then
    echo "$GH_MEMBERS" | jq -r '.org_url'
fi

if [ -n "$SHOW_SITE" ]; then
    echo "$GH_MEMBERS" | jq -r '.site'
fi
```
