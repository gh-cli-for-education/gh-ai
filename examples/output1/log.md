...
# Function to list branches using gh api command and fzf
list_branches() {
    local repository=$1
    
    # Make a GraphQL query to retrieve pull requests information
    ...
    # Print list of branches with colored pull request numbers
    echo "$query_result" | jq -r '.data.repository.pullRequests.nodes | sort_by(.number) | 
        map("\(.headRefName) \(.number | if . == 1 then "\u001b[32m" + "." + "\u001b[0m" else "\u001b[31m" + "." + "\u001b[0m" end) by fromjson.base64/author.login") | .[]'
}
...
```,The README.md file has been generated with the requested content. You can find a brief description of the program, an installation guide, the Usage section, and two examples demonstrating how to use the extension. 

If you need any further modifications or additions, feel free to let me know!
```
...
To be able to execute this program have been needed **5077** Tokens, broken down this way:

Tokens used by the program gh-ai: **4793**.
Tokens used by the LLM to generate the answers: **284**.
