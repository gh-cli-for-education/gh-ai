  ...
  while (!process.env.GRACEFUL_SHUTDOWN) {
    ...
    switch (RUN.status) {
      ...
      case 'requires_action':
        const TOOLS_REQUIRED = RUN.required_action.submit_tool_outputs.tool_calls
        .map((tool) => { 
          return tool.function.name; 
        });
        spinner.info(`${CONSOLE_PROMPT.OPENAI}The run requires an action from ${TOOLS_REQUIRED} tool(s). Waiting for the result.`);      
        return RUN.status;
      ...
      default: // In case there is an unknown status
        spinner.fail(`${CONSOLE_PROMPT.ERROR}Unknown status reached.`);
